...
int main(int argc, char **argv)
{
    ...
    auto node = std::make_shared<rclcpp::Node>("video");

    auto cloud_publisher = node->create_publisher<sensor_msgs::msg::PointCloud2>("/demo/cloud", rclcpp::QoS(100).transient_local());
    auto ground_publisher = node->create_publisher<sensor_msgs::msg::PointCloud2>("/demo/ground", rclcpp::QoS(100).transient_local());
    auto nonground_publisher = node->create_publisher<sensor_msgs::msg::PointCloud2>("/demo/nonground", rclcpp::QoS(100).transient_local());
    ...
    if (home != nullptr) {
        data_path = std::string(home) + data_path;
    }
    ...
    for (int n = init_idx; rclcpp::ok() && n < n_frames; ++n) {
        ...
        auto ground_msg = cloud2msg(pc_ground);
        auto nonground_msg = cloud2msg(pc_non_ground);
        cloud_msg.header.stamp = stamp;
        ground_msg.header.stamp = stamp;
        nonground_msg.header.stamp = stamp;

        cloud_publisher->publish(cloud_msg);
        ground_publisher->publish(ground_msg);
        nonground_publisher->publish(nonground_msg);
