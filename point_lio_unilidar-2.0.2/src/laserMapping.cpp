...
void standard_pcl_cbk(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &msg)
{
    ...
    if (stampSec(msg->header.stamp) < last_timestamp_lidar)
    {
    ...
    last_timestamp_lidar = stampSec(msg->header.stamp);

    PointCloudXYZI::Ptr ptr(new PointCloudXYZI());
    PointCloudXYZI::Ptr ptr_div(new PointCloudXYZI());

    double time_div = stampSec(msg->header.stamp);
    ...
    if (cut_frame)
    {
        ...
        for (int i = 0; i < ptr->size(); i++)
        {
            ...
            if (ptr->points[i].curvature / double(1000) + stampSec(msg->header.stamp) - time_div > cut_frame_time_interval)
            {
                if (ptr_div->size() < 1)
                    continue;
    ...
    else
    {
        ...
        time_buffer.emplace_back(stampSec(msg->header.stamp));
    }
...
void imu_cbk(const sensor_msgs::msg::Imu::ConstSharedPtr &msg_in)
{
    ...
    msg->header.stamp = timeFromSec(stampSec(msg_in->header.stamp) - time_lag_imu_to_lidar);
...
bool sync_packages(MeasureGroup &meas)
{
    ...
    {
        double imu_time = stampSec(imu_deque.front()->header.stamp);
        meas.imu.shrink_to_fit();
        while ((!imu_deque.empty()) && (imu_time < lidar_end_time))
        {
            imu_time = stampSec(imu_deque.front()->header.stamp);
...
void publish_init_kdtree(const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr &pubLaserCloudFullRes)
{
    ...
    laserCloudmsg.header.stamp = timeFromSec(lidar_end_time);
...
void publish_odometry(const rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr &pubOdomAftMapped)
{
    ...
    {
        odomAftMapped.header.stamp = timeFromSec(time_current);
    }
    else
    {
        odomAftMapped.header.stamp = timeFromSec(lidar_end_time);
    }
...
void publish_path(const rclcpp::Publisher<nav_msgs::msg::Path>::SharedPtr &pubPath)
{
    ...
    msg_body_pose.header.stamp = timeFromSec(lidar_end_time);
...
int main(int argc, char **argv)
{
    ...
    ros_node = std::make_shared<rclcpp::Node>("laserMapping");
    tf_broadcaster = std::make_shared<tf2_ros::TransformBroadcaster>(ros_node);

    readParameters(ros_node);
    cout << "lidar_type: " << lidar_type << endl;

    path.header.stamp = timeFromSec(lidar_end_time);
    ...
    auto sub_pcl = ros_node->create_subscription<sensor_msgs::msg::PointCloud2>(
        lid_topic, rclcpp::SensorDataQoS(), standard_pcl_cbk);

    auto sub_imu = ros_node->create_subscription<sensor_msgs::msg::Imu>(
        imu_topic, rclcpp::SensorDataQoS(), imu_cbk);

    auto pubLaserCloudFullRes = ros_node->create_publisher<sensor_msgs::msg::PointCloud2>("/pointlio/cloud_registered", 100000);

    auto pubLaserCloudFullRes_body = ros_node->create_publisher<sensor_msgs::msg::PointCloud2>("/pointlio/cloud_registered_body", 100000);

    auto pubLaserCloudEffect = ros_node->create_publisher<sensor_msgs::msg::PointCloud2>("/pointlio/cloud_effected", 100000);

    auto pubLaserCloudMap = ros_node->create_publisher<sensor_msgs::msg::PointCloud2>("/pointlio/laser_map", 100000);

    auto pubOdomAftMapped = ros_node->create_publisher<nav_msgs::msg::Odometry>("/pointlio/odom", 100000);

    auto pubPath = ros_node->create_publisher<nav_msgs::msg::Path>("/pointlio/path", 100000);

    auto plane_pub = ros_node->create_publisher<visualization_msgs::msg::Marker>("/pointlio/planner_normal", 1000);
    ...
    while (rclcpp::ok())
    {
        ...
        if (imu_en)
        {
            ...
            {
                while (Measures.lidar_beg_time > stampSec(imu_next.header.stamp))
                {
                    imu_last = imu_next;
                    imu_next = *(imu_deque.front());
                    imu_deque.pop_front();
        ...
        if (!use_imu_as_input)
        {
            ...
            for (k = 0; k < time_seq.size(); k++)
            {
                ...
                if (is_first_frame)
                {
                    if (imu_en)
                    {
                        while (time_current > stampSec(imu_next.header.stamp))
                        {
                            imu_last = imu_next;
                            imu_next = *(imu_deque.front());
                            imu_deque.pop_front();
                ...
                if (imu_en)
                {
                    bool imu_comes = time_current > stampSec(imu_next.header.stamp);
                    while (imu_comes)
                    {
                        imu_upda_cov = true;
                        ...
                        imu_next = *(imu_deque.front());
                        imu_deque.pop_front();
                        double dt = stampSec(imu_last.header.stamp) - time_predict_last_const;
                        kf_output.predict(dt, Q_output, input_in, true, false);
                        time_predict_last_const = stampSec(imu_last.header.stamp);
                        imu_comes = time_current > stampSec(imu_next.header.stamp);

                        {
                            double dt_cov = stampSec(imu_last.header.stamp) - time_update_last;

                            if (dt_cov > 0.0)
                            {
                                time_update_last = stampSec(imu_last.header.stamp);
        ...
        else
        {
            ...
            for (k = 0; k < time_seq.size(); k++)
            {
                ...
                while (time_current > stampSec(imu_next.header.stamp))
                {
                    ...
                    input_in.acc = input_in.acc * G_m_s2 / acc_norm;
                    double dt = stampSec(imu_last.header.stamp) - t_last;
                    ...
                    t_last = stampSec(imu_last.header.stamp);
                    imu_prop_cov = true;
                }
